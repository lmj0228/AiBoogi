...
const HeartIcon = ({ artId }) => {
  ...
  const toggleHeart = async () => {
    ...
    if (isLiked) {
      try {
        const response = await axios.post('http://10.125.121.206:8080/api/heart/unlike', null, { params: payload });
        const updatedHearts = hearts.filter(card => card !== artId);
        setHearts(updatedHearts);
        localStorage.setItem('likedHearts', JSON.stringify(updatedHearts));
        setIsLiked(false);
        console.log('request : ', response.config.url, response.config.params); //요청url과 파라미터 확인.
      } catch (error) {
        console.error('좋아요 취소 에러', error);
      }
    } else {
      try {
        const response = await axios.post('http://10.125.121.206:8080/api/heart/like', null, { params: payload });
